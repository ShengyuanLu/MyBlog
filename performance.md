## **延迟(latency)**和**吞吐量(throughput)**之间区别 (Difference between latency and throughput)
...
低延迟(latency)加低吞吐量(throughput)是个什么情况呢？那就是银行的柜员个个都是手脚勤快，处理每个顾客都是分分钟的事情，这就是低延迟。但是呢，不幸的是这个银行的员工比较少，所以就算这些柜员再能干，他们每小时处理的顾客也是比较有限的。
